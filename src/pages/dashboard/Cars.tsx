...
export default function Cars() {
  ...
  // Tracks which vehicles the user has just ordered in this session, so we
  // can show a "Processing" status on the card. This is a lightweight,
  // client-side status — swap for a real orders table + query if/when one
  // exists.
  const [orderedModels, setOrderedModels] = useState<Record<string, "Processing">>({});
  ...
  const sendDepositInstructionsEmail = async (
    ...
    // Look up first name for a personalized user-facing email, same as Deposit.tsx.
    ...
    // Confirmation + instructions email to the user.
    if (user.email) {
...
<p style="margin:0;">Your order status is currently <strong>Processing</strong>. We'll email you again once your deposit is confirmed.</p>`,
          },
        })
        .catch(() => {});
    }
...
<p style="margin:0;">Deposit amount due: <strong>${displayAmount}</strong></p>`,
  ...
  const handleConfirm = () => {
    ...
    setTimeout(() => {
      setConfirming(false);
      setSelected(null);
      setOrderedModels((prev) => ({ ...prev, [vehicle.model]: "Processing" }));
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        {vehicles.map((v) => {
          const shippingStatus = orderedModels[v.model];
          return (
            <article
              key={v.model}
            ...
            >
              ...
              <div className="p-3 md:p-6 flex-1 flex flex-col">
                ...
                <div className="mt-auto space-y-2">
                  ...
                  {/* Shipping status — shown once an order has been started */}
                  {shippingStatus && (
                    <div className="flex items-center justify-center gap-1.5 text-[11px] md:text-[12px] font-medium text-amber-600 bg-amber-500/10 border border-amber-500/30 rounded-full py-1.5">
                      <PackageCheck className="w-3 h-3 md:w-3.5 md:h-3.5" />
                      {shippingStatus}
                    </div>
                  )}
                </div>
            ...
            return (
              <div className="space-y-4">
                ...
                <p className="text-[12px] text-muted-foreground leading-relaxed">
                  ...
                  pre-filled. Your order will be activated once payment is confirmed. We'll also
                  email you the deposit instructions.
                </p>
